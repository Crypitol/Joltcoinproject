...
#if defined(NDEBUG)
# error "Joltcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9d7eb324d5b22afc0b58596a16f725c36eb742734dcc0c8ac4d223490cb149a4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Joltcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Joltcoin Signed Message:\n";
...
{
    // Joltcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Joltcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Joltcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Joltcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Joltcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Joltcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4c557c22707df7947f3f15c0d83cb796458bedf58c6966e6a46669338a88e7bc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "False Nuclear Attack Declared In Hawaii";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044c92a48e40127da74334d41d9230073e4d5266fe0941894b0f70eee67fd920613089ce95bebfe0254e1595f6252cd9dde1aa6704549b5caf031716d6c8f454c5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517010712;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086531874;

        if (fTestNet)
        {
            block.nTime    = 1517010663;
            block.nNonce   = 385400228;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7d5089f292735261340ebffa509c5a3cddf41856ca849bea28d9595536d447f6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc3, 0xb5, 0xda }; // Joltcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JoltcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JoltcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JoltcoinMiner(CWallet *pwallet)
{
    printf("JoltcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("joltcoin-miner");
    ...
    try { loop {
        ...
        printf("Running JoltcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JoltcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JoltcoinMiner, pwallet));
}
